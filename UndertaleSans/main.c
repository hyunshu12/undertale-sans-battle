...
static Sprite   gSprHeadDef, gSprHeadBlue, gSprHeadNoEye, gSprHeadClosed, gSprHeadTired; /* 머리 표정 */
...
void game_sans_body(const char* pose) { (void)pose; }   /* 팔 포즈는 Push2 */
...
static Sprite* sansHeadSprite(void) {
    ...
    return &gSprHeadDef;   /* Default/LookLeft/Wink 등 */
}
/* 샌즈 전신(다리+몸통+머리) 합성 렌더. 부품 바닥앵커, 중심 cx, 발끝 SANS_FEET. */
static void drawSans(void) {
    int cx = gSansX + (int)gSansOffsetX;
    Sprite* head = sansHeadSprite();
    int allOk = gSprLegs.ok && gSprTorso.ok && head->ok;
    if (allOk) {
        /* 다리(0.4773,1.0) / 몸통(0.5,1.0) / 머리(0.5,1.0) — BTS 좌표 */
        drawSpriteMag(&gSprLegs,  cx - 42, SANS_FEET - SANS_LEGS_H);        /* (cx-42,178) */
        drawSpriteMag(&gSprTorso, cx - SANS_TORSO_W / 2, 176 - SANS_TORSO_H);/* (cx-54,126) */
        drawSpriteMag(head,       cx - SANS_HEAD_W / 2, 128 - SANS_HEAD_H);  /* (cx-32, 68) */
        return;
    }
    /* 폴백: GDI 해골+몸통 박스 */
    {
...
static void freeResources(void) {
    ...
    freeSprite(&gSprHeadClosed); freeSprite(&gSprHeadTired);
