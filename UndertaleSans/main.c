...
static int    gDeathShatter = 0, gDeathSplit = 0;
...
static void startDeath(void) {
    ...
    gDeathT = 0.0f; gDeathShatter = 0; gDeathSplit = 0;
    gState = ST_GAMEOVER; stopBGM();
    /* BTS: 검은화면 0.666s 후 금간하트(HeartSplit), 다시 1.333s 후 산산조각 — 소리는 타이밍별로 */
}
...
static void update(float dt) {
    ...
    case ST_BATTLE:
        ...
        break;
    case ST_GAMEOVER:   /* BTS: 검은화면 → 0.666s 금간하트 → 2.0s 산산조각+6파편 → 4.0s 자동 메인메뉴 */
        gDeathT += dt;
        if (!gDeathSplit && gDeathT >= 0.666f) {   /* 금간 하트 등장 */
            gDeathSplit = 1; game_play_sound("HeartSplit");
        }
        if (!gDeathShatter && gDeathT >= 2.0f) {   /* 산산조각: 6파편 (BTS 속도180·랜덤각) */
            int si;
            gDeathShatter = 1;
            game_play_sound("HeartShatter");
            for (si = 0; si < 6; si++) {
                float a = frand(0.0f, 360.0f) * (float)(M_PI / 180.0);
                gShardX[si] = gDeathX; gShardY[si] = gDeathY;
                gShardVx[si] = (float)cos(a) * 180.0f;
                gShardVy[si] = (float)sin(a) * 180.0f;
            }
        }
        if (gDeathShatter) {
            int si;
...
static void render(void) {
    ...
    if (gState == ST_GAMEOVER) {   /* BTS: 검은화면 → 0.666s 금간하트 → 2.0s 산산조각 (GAME OVER 텍스트 없음) */
        if (gDeathT < 0.666f) {
            /* 검은 화면(아무것도 안 그림) */
        } else if (gDeathT < 2.0f) {
