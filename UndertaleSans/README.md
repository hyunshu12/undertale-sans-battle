...
| 키 | 동작 |
|---|---|
| 화살표 / WASD | 영혼(빨간 하트) 이동 |
| Z 또는 Enter | 시작 / 재시작 |
| ESC | 종료 |

## 현재 범위 (Slice 1)

타이틀 → 전투(좌우에서 날아오는 뼈를 피하며 HP 유지, 피격 시 1초 무적) → HP 0이면 게임오버 → 재시작.

이후 슬라이스에서 추가 예정: 게이스터 블래스터, 대사 타이핑, 샌즈 스프라이트, FIGHT/ACT/ITEM/MERCY 메뉴,
난이도 선택, 파란 영혼 점프, 4턴 아크 + 자비 엔딩, BGM(Megalovania), BTS 실제 스프라이트.
